    ...
    app.route('/pixelCompare')
        ...
        .post((req,res) =>  {   
            ...
            new Promise((resolve,reject)=>{
                await movetoServer('baselinePdfs',eq.files.baselinePdfFile);
                await movetoServer('actualPdfs',eq.files.actualPdfFile)
...
async function movetoServer(location, file){
    file.mv(process.cwd()+'/data/'+location+'/'+file+'.pdf', function(err){
        if (err) {
            console.log(err);
        }
    })
